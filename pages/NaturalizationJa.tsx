        ...
        },
        {
          ...
          mainEntity: [
            ...
            },
            {
              ...
              acceptedAnswer: {
                '@type': 'Answer',
                text: '書類1通のみは50,000円〜（税抜・送料実費別途）。帰化書類パック（本人・父母の出生証明書＋父母の婚姻証明書の計4通、アポスティーユ・DHL配送・税込）は132,500円で、兄弟姉妹の出生証明書などの追加は1通ごとに＋13,250円です。実例として家族分6通で159,000円。通数が増えても国際送料は1回分のみで、通数×単価の掛け算にはなりません。無料相談後に正確な金額をご提示します。',
              },
            },
            {
              '@type': 'Question',
    ...
    >
      ...
      <div className="overflow-hidden rounded-xl border border-gray-100 shadow-sm text-sm mt-8">
        ...
        {[
          ...
          { label: '帰化書類パック：本人・父母の出生証明書＋父母の婚姻証明書（計4通）＋アポスティーユ＋DHL配送', price: '132,500円（税込）', bold: true },
          { label: '兄弟姉妹の出生証明書など、パックへの追加1通ごと', price: '＋13,250円（税込）' },
          { label: '実例：家族6通（基本パック4通＋兄弟姉妹の出生証明書2通）', price: '159,000円（税込・一括配送込み）' },
        ].map((row, i) => (
      ...
      <p className="text-xs text-gray-500 mt-3 mb-8 leading-relaxed">
        ※ パック料金は<strong>通数×単価の掛け算ではありません</strong>。追加1通は＋13,250円と基本パックの1通あたり単価の半額以下で、全書類を1つの荷物に集約するため国際送料も1回分のみです。まとめてご依頼いただくほど1通あたりの単価は下がります。<br />
        ※ お支払いは着手金50%・完成書類のスキャンコピー確認後に残金50%。NBIクリアランスや翻訳、お急ぎ対応など内容により変わる場合があるため、正確なお見積りは無料相談でご提示します。
      </p>
      ...
      <FaqSection
        items={[
          ...
          { q: '料金はいくらですか？', a: '書類1通のみは50,000円〜（税抜・送料実費別途）。帰化書類パック（本人・父母の出生証明書＋父母の婚姻証明書の計4通、アポスティーユ・DHL配送・税込）は132,500円で、兄弟姉妹の出生証明書などの追加は1通ごとに＋13,250円です。実例として家族分6通で159,000円。通数が増えても国際送料は1回分のみで、通数×単価の掛け算にはなりません。無料相談後に正確な金額をご提示します。' },
