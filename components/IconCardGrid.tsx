...
export default function IconCardGrid({ heading, subheading, cards, columns = 3, className = '' }: IconCardGridProps) {
  return (
    <section className={`mb-10 ${className}`}>
      ...
      <div className={`grid gap-5 ${colsMap[columns]}`}>
        {cards.map((card) => {
          ...
          return (
            <div
              key={card.title}
              className="group bg-white rounded-2xl border border-gray-100 shadow-card p-6 hover:shadow-md hover:-translate-y-0.5 hover:border-primary/20 transition-all duration-200"
            >
              <div className={`w-10 h-10 rounded-xl ${accent} flex items-center justify-center mb-4 flex-shrink-0`}>
                <Icon className="w-5 h-5" />
              </div>
              <h3 className="text-base font-bold text-secondary mb-2 leading-snug">{card.title}</h3>
              {card.description && (
