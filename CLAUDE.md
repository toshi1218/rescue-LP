...
4. レバー分類: Level4個別 / Level4大量(20+) / Level3(title等) / 共通コンポーネント / 構造
   ＋【対象ページの実トラフィック】GSCで波及ページの impressions を確認し
   「高トラフィック / 低トラフィック（直近3ヶ月で実質ゼロ）」を明記する
5. ローリング7日バケツ: 「完了済み」表を見て、直近7日に大きいシグナル
   （共通コンポーネント・構造・20ページ超）を既に出していないか
6. 混入チェック: 意図しないファイル（useMeta.ts 等）が紛れていないか
7. 判定: 🟢 即OK / 🟡 単独デプロイ＋7日空けてOK / 🔴 分割 or 延期
```

- 判定基準の詳細（レバー×波及数→頻度テーブル、ローリング7日バケツ）は `docs/merge-schedule.md`「デプロイ頻度の判断基準」を正典とする。
- 🟡 でマージする場合は、同じ窓（7日）で他の共通コンポーネント・構造変更を出さないことを宣言してから実行する。
- **低トラフィックページのみの変更は判定を1段軽くしてよい**（下記「トラフィック軸」参照）。ただし軸1（変更量）の上限は据え置き。
...
## 📊 トラフィック軸（2026-09-23 実測で追加）

判定は **2軸** で行う。従来は軸1だけで見ていたため、失うものが無いページの修正にも
重いコスト（単独デプロイ・7日待ち）を払っていた。

| 軸 | 何を測るか | 何のリスクか |
|---|---|---|
| **軸1: 変更量** | ファイル数・波及ページ数・共通コンポーネント/構造への該当 | 「サイト全体が更新された」と誤認され全ページ再評価を誘発するリスク |
| **軸2: 実トラフィック** | 波及ページの直近3ヶ月 impressions（GSC実測） | 失敗したときに**実際に失うもの**の大きさ |

**運用ルール**:

- **軸2 が低い（波及ページの impressions が実質ゼロ）なら、判定を1段軽くしてよい**
  （🟡 → 🟢 など）。順位もクリックも無いページは、壊れても失うものが無い。
- **ただし軸1の上限は据え置く**。「1コミット20ファイル以上」「共通コンポーネント連打」は
  トラフィックに関わらず避ける。Googleの「サイト全体更新」検知は*変更量*を見ており、
  対象ページのトラフィックとは無関係だから。
- **軸2 が高いページを含む場合は、従来どおり重く扱う**。むしろこちらに慎重さを集中させる。

**測り方**: GSC の Pages を直近3ヶ月で開き、波及ページの impressions を引く。
ゼロ〜数十なら「低トラフィック」。数百以上を含むなら「高トラフィック」。

**実測の根拠（2026-09-23）**:

- #458（EN条約修正・16ページ波及・国別テンプレート含む）を 🟡 と判定し単独デプロイ扱いにしたが、
  対象ページの3ヶ月実績は `/en/saudi-arabia/` `/en/uae/` `/en/cenomar-requirements-by-country/`
  が **impressions ゼロ**、`/en/hong-kong/` 26・`/en/singapore/` 7・`/en/italy/` 10。
  例外は `/en/qatar/` 431 のみ。
- #464（法人4ページ）も `/ja/business/` 41impr、`/ja/business/gyoseishoshi/` はゼロ。
- つまり**コストだけ払って、守っているものが無かった**。
- 一方 #463（`/en/psa-birth-certificate-cost/` の title）は同ページ単体で
  サイト全 impressions の **15%（2,259/15,031）** を占める。こちらは重く扱う価値がある。

**副次的な効能**: 「正確性・法令・整合性のための修正」（誤情報訂正・価格統一・特商法など）は
そもそもKPIを動かす種類の変更ではない。これを効果測定の対象に混ぜると判断を誤るので、
**「数字を動かす変更」と「正しさのための変更」を分けて扱う**こと。

---

## 🤖 スケジュール自動マージの例外（2026-07-11 ユーザー承認で追加）

上記「main マージ絶対禁止」の**限定的な例外**として、**予定表（`docs/merge-schedule.md`）に「自動マージ可（低リスク）」と明記された事前検証済みPRに限り**、期日に発火する Routine による自動マージを許可する。恣意的・即時のマージ許可ではなく、**「予定表に載った低リスクPRのスケジュール実行」に限定**される。#273 の教訓（無計画な即マージの禁止）は維持する。

自動マージは次の安全条件を**すべて**満たす場合のみ実行する:

1. Routine が当日、最新 `origin/main` で**再検証**する（7項目カード）: リベース可能・`npm run build` 成功・波及ページ数を実測・レバー分類・ローリング7日バケツ・混入チェック。
2. 判定が **🟢** の時のみ自動マージ（squash）: 構造ファイル無変更／共通コンポーネント無変更／想定波及内／競合なし／混入なし。
3. **🟡・🔴・競合・main の想定外変動・構造ファイル混入・build 失敗**のいずれかがあれば、**自動マージせず「要確認」通知**して従来の手動承認（「main merge」入力）へ切り替える。
4. 自動マージ後は 7項目カードを添えて結果を通知し、「完了済み」表へ移動する。

**自動マージ対象外（常に手動「main merge」が必要）**:

- **共通コンポーネント**変更（Footer/WhyUs/Header/Hero/Nav/Pricing/CardGrid/FAQ 等）＝全ページ波及
- **構造ファイル**変更（本ファイル冒頭の定義）／20ページ超の一括
- **手動デプロイや外部設定が前提**のPR（例: `wrangler` デプロイ待ち）
- 現時点の該当: **#319（共通コンポーネント）・#313（手動デプロイ前提）は手動維持**
