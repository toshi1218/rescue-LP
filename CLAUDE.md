...
5. **真の下落要因（4/27 CEO ヒアリングで判明・仮説から事実に近づく）: Honeymoon 終了 × 情報ページ→営業ページの戦略転換**
   - 2/16-3/8 のピーク（494/日）は**情報ページ × Honeymoon ブーストの組合せ**で獲得
   - CEO が当時「CTR が低すぎる」と判断（`cenomar meaning` 等の informational query で 0 click 多発）→ **情報ページのコンテンツを営業ページに改修**（3月中旬）
   - その後さらに**情報ページと営業ページに分離**（4月）
   - したがって 494 → 37 の 92%減は以下の合算:
     - (a) Honeymoon Period 終了（不可避な自然減）
     - (b) 情報系クエリを**意図的に捨てた**（戦略選択：量より質）
     - (c) 営業ページが Google から信頼されるまでの再評価ラグ（過渡期・継続中）
