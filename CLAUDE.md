...
**実測影響**: 2026-05-26 ノビリスタで外免切替-3位下落、**2026-06-03〜06-04 ノビリスタ平均順位が再下落**（5/22大変更から+12〜14日＝余震ウィンドウと一致）  
**現在の制約**: 週1PR・1日1コミット・共通コンポーネント完全凍結・週2デプロイ

### 🔖 マージ保留中タスク（本番デプロイ待機）

**PR #287 / ブランチ `claude/nice-wozniak-q4R9U`**（NbiGuideJa 単体・1ファイル）

- **内容**: `/ja/nbi-clearance/` の郵送料誤記修正（+7,000円→+6,000円）＋納期訴求強化（自力2ヶ月以上 vs 当社約1ヶ月の比較ブロック）＋CTAを「無料で相談する」に統一。コードは完成・CI全グリーン
- **状態**: **マージ保留（ブランチで待機）**。1ファイル・本文/UIのみで構造的には安全だが、下げ局面に本番デプロイを足さない判断
- **保留理由（2026-06-04時点）**: ①ノビリスタが6/03〜6/04で再下落中（5/22余震の可能性）②今週すでに2回コミット済み ③回復期解除条件未達
- **マージ解除条件（両方そろったら）**:
  1. 6/03〜6/04の下落が戻る（ノビリスタで新たな-2以上の下落がない）
  2. 2026-06-19以降 ＋ GSC impressions が2週連続プラストレンド確認
- **やらないこと**: 他ページのCTA一括統一は**実施しない**（2026-06-04検証済み。`ctaService`紐付け・チェックリスト等の意図的な出し分けを壊すリスク＋本番33ファイル一斉更新シグナルに見合わないと判断）
