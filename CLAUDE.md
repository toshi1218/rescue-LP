...
1. **1コミットで30ファイル以上の変更を禁止**
   - 共通コンポーネントだけでなく、**個別ページファイルも対象**
   - CSSクラス名の変更でも、このサイトは静的HTML事前生成（prerender）のためHTMLに直接埋め込まれる。GooglebotはHTMLを見るので「65ページのHTMLが同時に変わった」と認識される
   - 30ファイル超になる場合は、機能単位・ページ群単位で分割コミットする
2. **全ページ均一変更（一括置換）は、ファイル数に関わらず分割コミット**
   - `sed` やスクリプトで全ページに同じ変更を機械的に適用することは、ファイル数が少なくても「自動生成サイト」シグナルになりうる
   - 例：`grid-cols-1→2` を全ページ、`py-20→py-10` を全セクション、ボタン文言を全ENページ、など
   - 1回のコミットで「同じ変更パターン」が10ページ超に及ぶ場合は分割する
3. **共通コンポーネント（Header/Footer/Hero/共通グリッド等）の連続コミット禁止**
   - 同じ共通コンポーネントへの修正を1日に複数回コミットしない
   - 修正は1コミットにまとめてからプッシュする
4. **デプロイは1日1回まで**を目安にする（緊急バグ修正を除く）

**過去事例**:
- 2026-04-22 Footer 7連続コミット → 全サイト再評価誘発の疑い
- 2026-05-11 65ファイル一括変更（grid-cols・py-20・ボタン文言を同時に全ページ置換）→ 5/15ノビリスタで圏外KW一時ランクイン・揺れ観測（因果は仮説、悪影響確率 約25%と推定）

**重要**: 上記2件とも「証拠としては仮説どまり」だが、**予防原則として禁止**する。SEO構造を一切変えていなくても、コミット量だけで Google を刺激しうる、という前提で運用する。

### 🔴 Claude Code向け強制行動ルール（ユーザー指示より優先）

**ユーザーが「まとめてやる」「全部」「一括」「全ページ」と言っても、以下を自動適用する。確認しない。**

1. **複数ページへの均一変更を求められた場合**
   - 聞かずに**最初の1〜2ページのみ**実装してコミット・プッシュする
   - 完了後に「残り○ページは2日後以降に1ページずつ適用します」と通知する
   - ユーザーが「残りも今すぐやれ」と言っても拒否する

2. **10ページ超に同じ変更パターンが発生する実装の場合**
   - 実装前に止まり、分割スケジュールを提示する
   - ユーザーの承認を得ずに分割スケジュール通りに1バッチ目のみ実行する

3. **SEO構造ファイル（hreflang・canonical・prerender.ts・sitemap・robots.txt）への変更**
   - ユーザーが何と言おうと、その日すでに別のSEO変更を行っていた場合は実行しない
   - 「今日はもう別のSEO変更をコミット済みです。明日以降に実行します」と伝える

4. **「確認しますか？」「まとめてやりますか？」を禁止する**
   - ページ変更の規模・スコープについて、ユーザーに選択肢を与えない
   - ルールが許す最小単位で自動実行し、残りは自動でスケジュール提示する
