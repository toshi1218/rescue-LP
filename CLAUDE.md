...
- **1ファイルの小さな修正（タイポ・コメント・スタイル調整）は単独PRにしない** — 次の関連変更と同じブランチにまとめてから1PRにする
- **PRは「論理的な作業単位」を1件とする** — 例：「CTAボタン改善」「SEOタイトル最適化」「新規ページ追加」
...
| 状況 | 対応 |
|------|------|
| 1箇所だけ直した | 次の関連修正と**まとめて**からPR |
| 1つのテーマの修正が完了した | PR作成 ✅ |
| 「とりあえず保存したい」 | コミットのみ・PRは後でまとめて作る |
